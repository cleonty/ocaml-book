...
(** [make_t n s] makes an OUnit test named [n] that expects
    [s] to fail type checking with type error string [s']. *)
let make_t n s' s =
  [n >:: (fun _ -> assert_raises (TypeError s') (fun () -> interp_small s));
   n >:: (fun _ -> assert_raises (TypeError s') (fun () -> interp_big s))]

(** [make_t n s] makes an OUnit test named [n] that expects
    [s] to fail type checking with runtime error string [s']. *)
let make_r n s' s =
  [n >:: (fun _ -> assert_raises (RuntimeError s') (fun () -> interp_small s));
   n >:: (fun _ -> assert_raises (RuntimeError s') (fun () -> interp_big s))]
