module type Set = sig
  ...
  (** [insert x s] is the set containg [x] as well as all the
      elements of [s]. *)
...
module ListSet : Set = struct
  (** AF: [[x1; x2; ...; xn]] represents the set {x1; x2; ..., xn}.
...
module BstSet : Set = struct
  (** AF: [Leaf] represents the empty set. [Node (l, v, r)] represents
          the set containg [v], as well as all the elements of the sets
          represented by [l] and [r].
      RI: for every node [Node (l, v, r)], all values in [l] are strictly 
          less than [v], and all the values in [r] are strictly great
          than [v] *)
  ...
  let rec insert x = function
    ...
    | Node(l, v, r) as n ->
      if x < v then Node(insert x l, v, Leaf)
      else if x > v then Node(Leaf, v, insert x r)
      else n
...
module RbSet : Set = struct 
  ...
  let insert x s =
    ...
    | Node (_, l, v, r) -> Node(Blk, l, v, r) (* color root Blk*)
end
